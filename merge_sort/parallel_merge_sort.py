...
def parallel_merge_sort(vector, key=lambda x:x, count_slice=0, self_id=1, return_dict=None):
    if return_dict == None:
    ...
    if count_slice >= 2:
        ...
        right_side_p.start()
        left_side = parallel_merge_sort(return_dict[self_id][:len(return_dict[self_id])//2], key, (count_slice//2), self_id*2, return_dict)
        right_side_p.join()
        return_dict[self_id] = merge(return_dict[self_id*2], return_dict[self_id*2+1], key)
    else:
        return_dict[self_id] = merge_sort(vector, key)
